...
```
1. 在 Parser 工作阶段，Parser 需要确定 fn 作用域情况，所以它会询问当前作用域有无 x 这个变量，没有的话就向**fn 所在作用域**中加入 x 这个变量（注意没有创建变量，作用域的目的是确定变量查找顺序，是语义上的，没有实际去操作内存，创建变量），否则就忽略该声明。
2. 在 fn 这个函数的上下文被创建的阶段，JavaScript 引擎（准确的说应该是 Ignition，因为代码的执行由它负责）会依据作用域**创建** x 这个变量 
3. 执行 x=2 
---
#### 变量声明在引擎的处理（存疑）
